...
//    })->name('orders.accepted');
    ...
    Route::view('/orders/accepted', 'accepted', ['orders' => \App\Models\Order::where('status_id', 1)->paginate(15)])->name('orders.accepted');

    Route::get('order/confirm/{id}', [])->name('confirm.order');

    Route::post('/order/cancel/{id}/{service_id}', [\App\Http\Controllers\HomeController::class, 'cancel.order']);
